 ...
 * @return APR_SUCCESS if all went well
 */
ngx_int_t (* read_domain)(int ids, domaininfo_t **domain);
/**
 ...
 */
ngx_int_t (*remove_domain)(domaininfo_t *domain);
/*
 * Insert a new domain or update existing one.
 */
ngx_int_t (*insert_update_domain)(domaininfo_t *domain);
/*
 * Find the domain using the JVMRoute and balancer information
 */
ngx_int_t (*find_domain)(domaininfo_t **node, const char *route, const char *balancer);
};
#endif /*DOMAIN_H*/
