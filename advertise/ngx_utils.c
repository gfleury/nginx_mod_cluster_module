...
#endif
static int
inet_pton6(const unsigned char *src, struct in6_addr *dst)
{
	static const char xdigits_l[] = "0123456789abcdef",
			  xdigits_u[] = "0123456789ABCDEF";
	unsigned char tmp[IN6ADDRSZ], *tp, *endp, *colonp;
	const char *xdigits;
    const unsigned char *curtok;
	int ch, saw_xdigit;
	unsigned int val;
    ...
    *port = 0; /* assume not specified */
    ...
    if (*str == '[') {
        ...
        if (scope_delim) {
            ...
            *scope_id = ngx_palloc(p, end_bracket - scope_delim);
