...
static mem_t * create_attach_mem_host(u_char *string, int *num, int type, ngx_pool_t *p, slotmem_storage_method *storage) {
    mem_t *ptr;
    ...
    ptr->storage =  storage;
    storename = ngx_pstrcat(p, string, HOSTEXE, NULL); 
    if (type)
        rv = ptr->storage->ap_slotmem_create(&ptr->slotmem, storename, sizeof(hostinfo_t), *num, type, p, storage->cf, storage->ngx_http_module);
    else {
        size_t size = sizeof(hostinfo_t);
...
ngx_int_t init_mem_host(mem_t *ptr, u_char *string, int *num, ngx_pool_t *p) {
    const u_char *storename;
    ngx_int_t rv;
    ...
    rv = ptr->storage->ap_slotmem_init(&ptr->slotmem, storename, sizeof(hostinfo_t), *num, p);
...
static ngx_int_t insert_update(void* mem, void **data, int id, ngx_pool_t *pool) {
    ...
    if (ngx_strcmp(in->host, ou->host) == 0 && in->vhost == ou->vhost && in->node == ou->node) {
...
ngx_int_t insert_update_host(mem_t *s, hostinfo_t *host) {
    ngx_int_t rv;
    hostinfo_t *ou;
    int ident;
 ...
 */
static ngx_int_t loc_read_host(void* mem, void **data, int id, ngx_pool_t *pool) {
    hostinfo_t *in = (hostinfo_t *)*data;
    hostinfo_t *ou = (hostinfo_t *)mem;

    if (ngx_strcmp(in->host, ou->host) == 0 && in->node == ou->node ) {
        *data = ou;
        return NGX_OK;
    }
    return NGX_NONE;
 ...
 * @return APR_SUCCESS if all went well
 */
ngx_int_t get_host(mem_t *s, hostinfo_t **host, int ids) {
 ...
 * @return APR_SUCCESS if all went well
 */
ngx_int_t remove_host(mem_t *s, hostinfo_t *host){
    ngx_int_t rv;
    hostinfo_t *ou = host;
    if (host->id)
 ...
 */
mem_t * get_mem_host(u_char *string, int *num, ngx_pool_t *p, slotmem_storage_method *storage) {
 ...
 * @return address of struct used to access the table.
 */
mem_t * create_mem_host(u_char *string, int *num, int persist, ngx_pool_t *p, slotmem_storage_method *storage) {
